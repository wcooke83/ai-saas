...
| 7 | e2e-admin-trials | ✅ Done | ✅ | | | | 0 | | | 20/20 passed on rerun (was 17/20; 3 plan-dependent tests now pass — DB has subscription_plans) | 2026-04-04 |
...
| 27 | e2e-chatbot-crud | ✅ Done | | 🔧 | | 0 | Timeout / State | Test Code | 3/3 | will | networkidle wait added to create test (was domcontentloaded — page showed Loading... spinner); all 6/6 pass on rerun | 2026-04-04 |
...
| 36 | e2e-credit-purchase | ✅ Done | | 🔧 | | 0 | Selector | Test Code | 3/3 | CREDIT-001–004 fixed on rerun: seeded E2E package in beforeAll/afterAll; fixed h3 parent selector; CREDIT-002 strict mode .first() | 2026-04-04 |
...
| 49 | e2e-deployment-page | ✅ Done | | 🔧 | | 0 | Timeout | Test Code | 3/3 | will | DEPLOY-022 fixed: enable live_handoff via API before test, disable after; flaky on first (cold compile), passes on retry | 2026-04-04 |
...
| 73 | e2e-navigation-onboarding | ✅ Done | | 🔧 | | 0 | State/Data | Test Code | 1/3 | will | NAV-004/005/006/LAYOUT-005 fixed: DELETE /publish before test (allComplete→false → checklist shows); removed "Test Your Chatbot" step (only 3 steps exist); LAYOUT-005 tests deploy step; NAV-006 flaky→retry | 2026-04-04 |
...
### Remaining Skips (cannot fix without design/server change)
- **e2e-article-generation AG-040**: live_fetch embedding mismatch — needs server-side fix
- **e2e-chatbot-booking-page BOOKING-010/011**: "Appointment Booking" nav link in public header — UX/product decision
- **e2e-chatbot-widget-deploy-page** (1 test): back-to-chatbot link removed from deploy page — design decision
