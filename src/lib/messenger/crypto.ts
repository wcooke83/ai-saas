...
export function encryptMessengerConfig<T extends Record<string, unknown>>(config: T): T {
...
export function decryptMessengerConfig<T extends Record<string, unknown>>(config: T): T {
  if (!config.access_token || typeof config.access_token !== 'string') return config;
