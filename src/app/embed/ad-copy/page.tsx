...
import type { AdCopyOutput } from '@/lib/ai/prompts/ad-copy';

function AdCopyEmbed() {
  ...
  useEffect(() => {
    ...
    if (window.parent !== window) {
      window.parent.postMessage(
        { source: 'ai-saas-embed', type: 'ready', toolId: 'ad-copy' },
        '*'
      );
    }
  ...
  const handleGenerate = (result: AdCopyOutput) => {
  ...
  return (
    <div className="min-h-screen p-4" style={{ backgroundColor: 'rgb(var(--card-bg))' }}>
      <AdCopyGenerator apiKey={apiKey || undefined} onGenerate={handleGenerate} />
...
export default function AdCopyEmbedPage() {
  return (
    <Suspense
      fallback={
