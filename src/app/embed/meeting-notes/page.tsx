...
import type { MeetingSummary } from '@/lib/ai/prompts/meeting-notes';

function MeetingNotesEmbed() {
  ...
  useEffect(() => {
    ...
    if (window.parent !== window) {
      window.parent.postMessage(
        { source: 'ai-saas-embed', type: 'ready', toolId: 'meeting-notes' },
        '*'
      );
    }
  ...
  const handleGenerate = (result: MeetingSummary) => {
  ...
  return (
    <div className="min-h-screen p-4" style={{ backgroundColor: 'rgb(var(--card-bg))' }}>
      <MeetingNotesSummarizer apiKey={apiKey || undefined} onGenerate={handleGenerate} />
...
export default function MeetingNotesEmbedPage() {
  return (
    <Suspense
      fallback={
